...
cmd = f'ffmpeg -i {origin_uuid_path}/{name} -b:v:0 4000k -maxrate 4500k -bufsize 9000k -b:a:0 192k -ac 2 -map 0:v -map 0:a -f hls -var_stream_map "v:0,a:0"  -hls_list_size 0 -master_pl_name master.m3u8 -hls_segment_filename "{transcode_uuid_path}/vs%v/file_%03d.ts" -force_key_frames "expr:gte(t,n_forced*1)" -hls_time 2 -pix_fmt yuv420p {transcode_uuid_path}/vs%v/out.m3u8'
